#include "WorldSystem_old.h"
#include "Defined.h"
#include "GameToolkit.h"
WorldSystem_old* WorldSystem_old::instance = nullptr;

WorldSystem_old* WorldSystem_old::getInstance()
{
	if (instance == nullptr) {
		instance = new WorldSystem_old();
		if (instance == nullptr) {
#ifdef _DEBUG
			std::cout << "\t\tERROR:new WorldSystem_old()申请内存失败，值为nullptr\n";
#endif //_DEBUG
		}
	}
	return instance;
...
void WorldSystem_old::destroyInstance()
{
	delete this;
}

void WorldSystem_old::removeChara(Chara_old* chara)
{
#ifdef WORLD_SYSTEM_DEBUG
	std::cout << "WorldSystem_old::removeChara(Chara_old* chara)\n";
	if (chara == nullptr) {
...
Facility_old* WorldSystem_old::addFacility_static(Facility_old* facility)
{
	faciSys->facilityList_static->push_back(facility);
	return facility;
}

void WorldSystem_old::removeFacility_static(Facility_old* facility)
{
	faciSys->facilityList_static->remove(facility);
}

void WorldSystem_old::logicGo()
{
	projLogicGo();
	faciLogicGo();
	charaLogicGo();
...
void WorldSystem_old::charaLogicGo()
{
	//遍历角色列表
	...
	while (true)
	{
		...
		if (chara->actionType == ActionType::falling &&
			chara->fallingY == 0) {
			if (gameToolkit::ifFalling(chara)) {
				chara->fallingY += 1;
				chara->fallingDist += 1;
			}
			else
...
void WorldSystem_old::faciLogicGo()
{
...
void WorldSystem_old::projLogicGo()
{
	//投射物循环
...
void WorldSystem_old::particleLogicGo()
{
	//(1/2)文字粒子遍历
...
WorldSystem_old::WorldSystem_old()
{
